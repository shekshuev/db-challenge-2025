...
На олимпиаде по базам данных участникам предлагается решить 7 задач в течение 3 часов.

Результатом решения задачи скрипт SQL или PL/pgSQL, при запуске которого получается верное решение.

## Правила проведения Олимпиады:

1. Перед тем, как приступить к выполнению заданий, участник обязан заполнить карточку участника, указав там свои фамилию, имя, отчество и номер группы.
2. Доступ к базе данных осуществляется удаленно с помощью утилиты psql или графических приложений pgAdmin или DBeaver. Реквизиты доступа к серверу (адрес, логин и пароль) указаны в карточке участника.
3. Участник обладает правами владельца БД относительно той БД, к которой он подключается.
4. Участнику запрещено удалять уже имеющиеся на момент начало олимпиады объекты БД.
5. При «поломке» целевой БД по вине участника новая БД ему не предоставляется.
6. Участникам запрещается пользоваться любыми личными съемными носителями информации, телефонами и другими устройствами, учебной литературой, заготовленными личными записями, сетью Интернет и иными непредусмотренными заданиями сетевыми ресурсами. **Разрешается использовать официальную документацию PostgreSQL**.
7. В ходе каждого тура участники могут задавать вопросы по условиям задач. Возможные варианты ответов члена жюри: "ДА", "НЕТ" или "БЕЗ КОММЕНТАРИЕВ". Жюри оставляет за собой право объявить ответ на вопрос всем остальным участникам олимпиады.
8. Во время тура участники имеют право общаться только с жюри.

## Правила представления решений

Участник должен создать таблицу в БД следующего формата:

| Номер задания | Решение |
| ------------- | ------- |
| ...           | ...     |

В колонке `Номер задания` указывается номер или полное название задания, в колонке `Решение` - SQL или PL/pgSQL скрипт.

## Правила оценивания решений

Решения участников оцениваются составом жюри на следующий день после олимпиады с присуждением определенного количества баллов за каждую решенную задачу. При равенстве баллов у двух и более участников приоритет у того, кто написал наиболее оптимальное решение (по времени выполения, плану запроса, количеству строк кода).
